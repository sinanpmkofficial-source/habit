  ...
  return (
    <div className="w-full max-w-md mx-auto flex flex-col gap-6 px-4 py-6 md:py-8">
      ...
      <div className="flex flex-col">
        ...
        <p className="text-xs text-muted-text mt-0.5">Visualize your habits calendar</p>
      </div>
        ...
        {habits.length > 0 && (
          <div className="flex flex-col">
            ...
            <select
              ...
              className="w-full px-3 py-2.5 rounded-xl border border-border bg-card-bg text-sm font-semibold focus:outline-none focus:border-black dark:focus:border-white transition-all text-black dark:text-white"
            >
              {habits.map((habit) => (
      ...
      {activeHabit ? (
        <div className="grid grid-cols-2 gap-2 select-none">
          ...
          <div className="flex flex-col p-3 rounded-2xl border border-border bg-card-bg">
            ...
            <span className="text-xl font-extrabold text-black dark:text-white mt-1.5">
              {stats.completionRate}%
            </span>
            <span className="text-[9px] text-muted-text mt-0.5">For this month</span>
          </div>
          ...
          <div className="flex flex-col p-3 rounded-2xl border border-border bg-card-bg">
            ...
            <span className="text-[9px] text-muted-text mt-0.5">Completed in {MONTHS[viewMonth]}</span>
          </div>
          ...
          <div className="flex flex-col p-3 rounded-2xl border border-border bg-card-bg">
            <div className="flex items-center gap-1.5 text-muted-text">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider text-black dark:text-white">Streak</span>
            </div>
            <span className="text-xl font-extrabold text-black dark:text-white mt-1.5">
              {stats.currentStreak} <span className="text-xs font-normal text-muted-text">days</span>
            </span>
            <span className="text-[9px] text-muted-text mt-0.5">Current active streak</span>
          </div>
          ...
          <div className="flex flex-col p-3 rounded-2xl border border-border bg-card-bg">
            ...
            <span className="text-[9px] text-muted-text mt-0.5">All-time record</span>
          </div>
        </div>
      ) : null}
      ...
      {!activeHabit ? (
        ...
        <div className="flex flex-col p-4 rounded-2xl border border-border bg-card-bg">
          {/* Weekday labels */}
          <div className="grid grid-cols-7 text-center mb-2">
            {WEEKDAYS.map((day, idx) => (
              <span key={idx} className="text-[10px] font-bold text-muted-text">
                {day}
              </span>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 gap-y-1 text-center">
            {calendarDays.map((cell, idx) => {
              ...
              return (
                <div key={cell.dateStr} className="aspect-square flex items-center justify-center p-0.5">
                  <button
                    disabled={!canToggle}
                    onClick={() => toggleHabitCompletion(activeHabit._id!, cell.dateStr)}
                    className={`btn-interactive w-full aspect-square rounded-full flex flex-col items-center justify-center text-xs font-bold border relative ${
                      isCompleted
                        ...
                        : "border-transparent text-black dark:text-zinc-100 hover:border-zinc-300 dark:hover:border-zinc-700"
                    } ${
                      !canToggle
