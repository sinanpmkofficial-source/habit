  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col gap-6 px-4 py-6 md:py-8">
      <div className="flex flex-col">
        ...
        <p className="text-xs md:text-sm text-muted-text mt-0.5">Manage habits and application data</p>
      </div>

      {/* Import Status Alert */}
      {importStatus && (
        <div
          className={`p-3 rounded-xl border text-xs md:text-sm font-medium animate-fade-in ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onAddHabit}
              className="text-xs md:text-sm font-bold text-black dark:text-white hover:underline cursor-pointer"
            >
              + Add New
            </button>
          </div>
          ...
          {habits.length === 0 ? (
            <div className="p-4 border border-dashed border-border rounded-2xl text-center bg-card-bg text-xs md:text-sm text-muted-text">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                ...
                className="btn-interactive flex items-center justify-center gap-2 p-3.5 rounded-2xl border border-border bg-card-bg hover:border-black dark:hover:border-white text-xs md:text-sm font-bold text-black dark:text-white"
              >
            ...
            {!showDeleteConfirm ? (
              <button
                ...
                className="btn-interactive flex items-center justify-center gap-2 p-3.5 rounded-2xl border border-red-100 hover:border-red-200 bg-red-50/10 text-xs md:text-sm font-bold text-red-600 hover:bg-red-50/50 dark:bg-red-950/5 dark:border-red-950/20"
              >
              ...
              <div className="flex flex-col p-4 border border-red-200 dark:border-red-900/40 rounded-2xl bg-red-50/10 dark:bg-red-950/5 gap-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="btn-interactive flex-1 p-2 rounded-xl bg-red-600 text-white text-xs md:text-sm font-bold hover:bg-red-700"
                  >
                    Confirm Delete
                  </button>
                  <button
                    onClick={() => setShowDeleteConfirm(false)}
                    className="btn-interactive flex-1 p-2 rounded-xl border border-border bg-card-bg text-black dark:text-white text-xs md:text-sm font-bold"
                  >
                    Cancel
                  </button>
                </div>
